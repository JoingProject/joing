...
public abstract class PDETaskBarComponent extends PDEDeskComponent implements TaskBarComponent
{
    ...
    { 
        ...
        {   
            ...
            itemPreferences.setIcon( new ImageIcon( PDEUtilities.getDesktopManager().getRuntime().getImage( StandardImage.PROPERTIES, 16, 16 ) ) );
            add( itemPreferences );
            
            itemAbout = new JMenuItem( "About" );
            itemAbout.addActionListener( this );
            itemAbout.setIcon( new ImageIcon( PDEUtilities.getDesktopManager().getRuntime().getImage( StandardImage.INFO, 16, 16 ) ) );
            add( itemAbout );
            ...
            itemRemove.setIcon( new ImageIcon( PDEUtilities.getDesktopManager().getRuntime().getImage( StandardImage.REMOVE, 16, 16 ) ) );
            add( itemRemove );
            
            itemMove = new JMenuItem( "Move" );
            itemMove.addActionListener( this );
            itemMove.setIcon( new ImageIcon( PDEUtilities.getDesktopManager().getRuntime().getImage( StandardImage.MOVE, 16, 16 ) ) );
            itemMove.setEnabled( ! isLocked() );
            add( itemMove );
            
            itemLock = new JMenuItem( isLocked() ? "Unlock" : "Lock" );
            itemLock.addActionListener( this );
            itemLock.setIcon( new ImageIcon( PDEUtilities.getDesktopManager().getRuntime().getImage( StandardImage.LOCK, 16, 16 ) ) );
            add( itemLock );
        }
