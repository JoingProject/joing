 ...
 * but WITHOUT ANY WARRANTY; withouit even the implied warranty of
 ...
 */
/**
 * Eliminamos toda referencia a JFrame
 * Agregamos una referencia dinamica para el  JDialog 
 *  SwingUtilities.getWindowAncestor(this)
 * 
 * metimos código al finalize.
 */
 ...
 */
public class JoingPDFViewer extends JPanel
        implements KeyListener, TreeSelectionListener,
        PageChangeListener, DeskComponent {
    ...
    PagePanel fspp;
    /** New Panel for Joing compliance*/
    JPanel panelTB;
    //    Thread anim;
    ...
    public ImageIcon getIcon(String name) {
        ...
        URL url = null;
        try {            
            url = getClass().getResource("/com/sun/pdfviewer/gfx/" + name);
            //PDFViewer.class.getResource("/com/sun/pdfview/gfx"+);
     ...
     */
    public JoingPDFViewer(boolean useThumbs) {        
        doThumb = useThumbs;
        init();
    }
    ...
    protected void init() {
        ...
        //thumbAction
        JButton jbMOpen;
        jbMOpen = new JButton(openAction);
        jbMOpen.setText("Open...");
        JButton jbMClose;
        jbMClose = new JButton(closeAction);
        jbMClose.setText("Close");
        JButton jbMPageSetup;
        jbMPageSetup = new JButton(pageSetupAction);
        jbMPageSetup.setText("Page Setup");
        JButton jbMPrint;
        jbMPrint = new JButton(printAction);
        jbMPrint.setText("Print");
        JButton jbMHideThum;
        jbMHideThum = new JButton(thumbAction);
        jbMHideThum.setText("Hide thumbnails");
        toolbarMenu.add(jbMOpen);
        toolbarMenu.add(jbMClose);
        toolbarMenu.add(Box.createVerticalGlue());
        toolbarMenu.add(jbMPageSetup);
        toolbarMenu.add(jbMPrint);
        toolbarMenu.add(Box.createVerticalGlue());
        toolbarMenu.add(jbMHideThum);
        //--End new toolbar.

        panelTB = new JPanel();
        panelTB.setLayout(new BorderLayout());
        panelTB.add(toolbarMenu, BorderLayout.NORTH);
        panelTB.add(toolbar, BorderLayout.SOUTH);
        //se agrega el toolbar.
        this.add(panelTB, BorderLayout.NORTH);
...
//        System.out.println("Going to page " + pagenum);
    ...
    public void openFile(File file) throws IOException {
        ...
        if (outline != null) {
            if (outline.getChildCount() > 0) {
                olf = new JDialog(SwingUtilities.getWindowAncestor(this), "Outline");
    ...
    public void doOpen() {
        try {
            JoingFileChooser fc =new JoingFileChooser() ; 
            //JFileChooser fc = new JFileChooser();
            fc.setCurrentDirectory(prevDirChoice);
            fc.setFileFilter(pdfFilter);
            fc.setMultiSelectionEnabled(false);
            int returnVal = fc.showOpenDialog(this);
            if (returnVal == JoingFileChooser.APPROVE_OPTION) {
                try {
    ...
    class PrintThread extends Thread {
        ...
        public void run() {
            try {
                ...
                JOptionPane.showMessageDialog(JoingPDFViewer.this,
    ...
    class PerformFullScreenMode implements Runnable {
        ...
        public void run() {
            ...
            fspp.addKeyListener(JoingPDFViewer.this);
            gotoPage(curpage);
...
//        }
//        // start the viewer
//        JoingPDFViewer viewer;
//        viewer = new JoingPDFViewer(useThumbs);
//        if (fileName != null) {
//            viewer.doOpen(fileName);
//        }
        JoingPDFViewer jpv = new JoingPDFViewer();
        jpv.showInFrame();
    }
    ...
    public void showInFrame() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                frame.setTitle("SwingLabs PDF Viewer. Joing Edition by RuGI"); 
                frame.setIcon(JoingPDFViewer.this.getIcon("pdf.gif").getImage());
                frame.add(JoingPDFViewer.this);     
                JoingPDFViewer.this.deskMgr.getDesktop().getActiveWorkArea().add(frame);
                //-- Termina parte Joing
                //--
                //--
                //--Inicia parte Frame normal
//                JFrame f = new JFrame();                                
//                f.setTitle("PDF VIewer");
//                f.setIconImage(JoingPDFViewer.this.getIcon("pdf.gif").getImage());
//                f.pack();
//                f.getContentPane().add(JoingPDFViewer.this);
//                f.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
//                f.setVisible(true);
                //--Termina parte Frame normal.
            }
        });
    ...
    public void finalize() {
        //System.out.println("Entro al finalize...");
        //doQuit();
    }

    public void dispose() {
        //System.out.println("Entre al dispose.");
        //super.dispose();
